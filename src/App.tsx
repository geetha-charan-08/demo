...
export default function App() {
  ...
  async function startListening() {
    ...
    if (!Recognition) { setVoiceReady(false); setToast('Voice input needs Chrome or Edge. You can still type every answer below.'); return; }
    try {
      ...
      recognition.lang = language.code; recognition.interimResults = false; recognition.continuous = false;
      ...
      recognition.onerror = () => { setIsListening(false); setToast('I could not hear that. Check microphone permission, move somewhere quiet, or type your answer.'); };
      recognitionRef.current = recognition; setIsListening(true); recognition.start();
    } catch {
      setIsListening(false); setVoiceReady(false); setToast('Microphone permission was blocked. Allow microphone access in your browser, then try again or type your answer.');
    }
  }
  ...
  return <div className="app-shell">
    ...
    {view === 'assistant' && <Assistant language={language} messages={messages} input={input} setInput={setInput} isListening={isListening} isSpeaking={isSpeaking} voiceReady={voiceReady} progress={progress} step={step} saving={saving} onListen={startListening} onAnswer={answerQuestion} onSpeak={() => { const last = messages[messages.length - 1]; if (last) speak(last.text); }} onReset={() => { setStep(-1); setMessages([]); setInput(''); setVoiceReady(true); setView('home'); }} />}
